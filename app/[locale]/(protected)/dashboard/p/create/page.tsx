...
export default function CreatePropertyPage() {
  ...
  const validateForm = (): { isValid: boolean; errors: Record<string, string> } => {
    ...
    // Use current data from ref for validation
    const currentData = formDataRef.current;
  ...
  const validateCurrentStep = (): boolean => {
    const stepValidations: Record<number, (keyof CreatePropertyFormData)[]> = {
      ...
      3: [] // Images are validated in step completion check
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(isRTL ? 'تم إنشاء العقار بنجاح!' : 'Property created successfully!');
      ...
      alert(isRTL ? `خطأ في إنشاء العقار: ${errorMessage}` : `Error creating property: ${errorMessage}`);
    } finally {
      setLoading(false);
    }
  };
          ...
          <div className="flex items-center justify-between">
            <div>
              <h1 className="bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-4xl font-bold text-foreground">
                {isRTL ? 'إنشاء عقار جديد' : 'Create New Property'}
              </h1>
              <p className="mt-2 text-lg text-muted-foreground">
                {isRTL ? 'إضافة عقار جديد إلى المنصة' : 'Add a new property to the platform'}
              </p>
            </div>
                    ...
                    <button
                      ...
                      className={`flex size-12 items-center justify-center rounded-full border-2 transition-all ${
                        isActive 
                          ...
                          : canNavigate
                          ? 'border-muted-foreground bg-background text-muted-foreground cursor-pointer hover:border-primary/50'
          ...
          <div className={`flex justify-between ${isRTL ? 'flex-row-reverse' : ''}`}>
            ...
            <div className="flex gap-3">
              ...
              {currentStep < steps.length ? (
                ...
                <Button 
                  type="submit" 
                  disabled={loading || !stepCompleted[1] || !stepCompleted[2] || !stepCompleted[3]}
                  className="bg-gradient-to-r from-green-600 to-blue-600 hover:from-green-700 hover:to-blue-700"
                >
                  {loading ? (
                    <>
                      <Save className="mr-2 size-4 animate-spin" />
                      {isRTL ? 'جاري إنشاء العقار...' : 'Creating Property...'}
                    </>
                  ) : (
                    <>
                      <CheckCircle2 className="mr-2 size-4" />
                      {isRTL ? 'إنشاء العقار' : 'Create Property'}
                    </>
                  )}
                </Button>
              )}
