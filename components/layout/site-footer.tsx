...
export function SiteFooter({ className }: React.HTMLAttributes<HTMLElement>) {
  ...
  const getDefaultCopyright = () => {
    const year = new Date().getFullYear();
    return isRTL 
      ? `© ${year} العقارية. جميع الحقوق محفوظة`
      : `© ${year} RealEstate. All rights reserved`;
  };

  const brandDescription = isRTL 
      ...
      <div className="container mx-auto px-4">
            ...
            {footerData?.showSocialLinks && footerData.socialLinks && (
              <div className="flex gap-3">
                {footerData.socialLinks.map((social) => {
                  const platform = social.platform.toLowerCase() as keyof typeof SOCIAL_ICONS;
        ...
        <div className="flex flex-col items-center justify-between gap-4 border-t border-[#555555] pt-8 md:flex-row">
          <p className="text-sm text-gray-500">
            {footerData?.copyrightText || getDefaultCopyright()}
          </p>
          <ModeToggle />
        </div>
      </div>
